...
class Shell:
    """Implements the home-grown shell"""
...
def get_parser(
    filename: str, start_rule: str, parsing_method: str = "earley"
) -> Lark:
    """Get the lark parser from the given parameters.

    Args:
        filename (str): The name of the grammar file
        start_rule (str): The name of the start rule of such grammar
...
def main():
    ...
    We have modified the interactive mode such that,
    ...
    Raises:
        ValueError: Raises when wrong number of arguments are given to the
