...
class Converter(Interpreter, metaclass=ABCMeta):  # pragma: no cover
    ...
    def convert(self, tree: Tree):
        """Convert the syntax tree to another data structure"""
        pass


class CommandConverter(Converter):
    """Implements the abstract Converter class"""

    def convert(self, tree: Tree) -> Command:
        """Implements the abstract convert method

        Convert the syntax tree to a Command instance
        Args:
...
class ConverterDecorator(Converter):  # pragma: no cover
    """The base decorator class of Converter"""
    def __init__(self, converter: Converter) -> None:
        super().__init__()
        self._converter: Converter = converter

    def convert(self, tree: Tree):
        """Implements the abstract method, provide default operation"""
...
class BackquoteDecorator(ConverterDecorator):
    ...
    The decorator of CommandConverter.
    Adding functionality to convert back quote.
    """
    def convert(self, tree: Tree) -> List:
        """Convert the syntax tree to list for the first parse
