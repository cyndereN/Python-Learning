...
class Visitor(metaclass=ABCMeta):  # pragma: no cover
    """Visitor Interface for Command Class"""

    @abstractmethod
    def visit_call(self, call, out):
        """Evaluates the Call instances"""
        pass
...
class Evaluator(Visitor):
    ...
    def visit_call(self, call, out: Deque) -> None:
        """Implements the visit_call method in Visitor.
    ...
    def visit_sequence(self, sequence, out: Deque) -> None:
        """Implements the visit_sequence method in Visitor.

        Args:
            sequence (Sequence): The instance of Sequence to be evaluated
            out (Deque): The deque that holds the strings as output
        """
        for command in sequence.command_sequence:
            command.accept(self, out)

    def visit_pipe(self, pipe, out: Deque) -> None:
        """Implements the visit_pipe method in Visitor.
